...
#filepath_model_seg_LaneLine = './models/model_147_lane_512_256_p2_q3_1206.pth'
...
video_path='./video/Front/(Front)IPCamera_20200305102504_p011.avi'
savefilename='./results/(Front)IPCamera_20200305102504_p011'

video_t_start = 0 # unit: second
video_t_end = 12 # unit: second

partial_inference_video =[[0, 1*60],[1*60, 2*60],[2*60, 3*60]]
#partial_inference_video = [[0,4],[4,8],[8,12]]
...
def fun_capture_fileapth(foldedecision_boxesr_path1, folder_path2):
    filename_path1=[]
    filename_path2=[]
...
def thread_detect(model_od, frame_pil_img, q_detect):
    _, bboxes = detect(model_od, frame_pil_img, min_score=0.3, max_overlap=0.5, top_k=50,device=device)
    q_detect.put(bboxes)
...
def thread_seg_models(model_seg_road, model_seg_lane ,frame_pil_img, q_sed):
    #argmax_feats_road,argmax_feats_lane, color_map_display_road, color_map_display_lane = evaluateModel_models(model_seg_road,model_seg_lane, frame_pil_img, BGRimg,inWidth=512, inHeight=256, device=device)
...
for c_time, tmp_time in enumerate(partial_inference_video):
    ...
    #DIVX (avi) MPEG mp4v
    # videoWriter = cv2.VideoWriter(savevideoname, cv2.VideoWriter_fourcc(*'mp4v'), fps, size)
    ...
    while success:
        if (c >= video_t_start*fps) & (c<=video_t_end*fps):
            ...
            BGRimg = frame_np_img.copy() #for cv2
