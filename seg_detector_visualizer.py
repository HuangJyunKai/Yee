...
class SegDetectorVisualizer(Configurable):
    ...
    def demo_visualize(self, image_path, output):
        ...
        for box in boxes:
            box = np.array(box).astype(np.int32).reshape(-1, 2)
            boxmax = np.amax(box,axis=0)+15
            boxmin = np.amin(box,axis=0)-15
            decision_box = [boxmin[0], boxmin[1],boxmax[0], boxmax[1]] #xmin ymin xmax ymax
            print("decision_box:",decision_box)
            xminl.append(boxmin[0])
            yminl.append(boxmin[1])
            xmaxl.append(boxmax[0])
            ymaxl.append(boxmax[1])
            cv2.rectangle(pred_canvas, (boxmin[0], boxmin[1]), (boxmax[0], boxmax[1] ), (0, 0, 255), 2)
            #cv2.polylines(pred_canvas, [tmp], True, (0, 255, 0), 2)
        xmin = min(xminl)
        ymin = min(yminl)
        xmax = max(xmaxl)
        ymax = max(ymaxl)
        decision_boxbig = [xmin, ymin,xmax,ymax]
        print("decision_boxbig:",decision_boxbig)
        cv2.rectangle(pred_canvas, (xmin, ymin), (xmax, ymax ), (255, 0, 0), 2)
        return pred_canvas
